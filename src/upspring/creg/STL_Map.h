...
namespace creg
{
	...
	struct DeduceType < std::map<TKey, TValue> > {
		IType* Get () {
			...
			return new MapType < std::map <TKey, TValue> > (keytype.Get(), valuetype.Get());
		}
	};
	// Multimap
	template<typename TKey, typename TValue>
	struct DeduceType < std::multimap<TKey, TValue> > {
		IType* Get () {
			DeduceType<TValue> valuetype;
			DeduceType<TKey> keytype;
			return new MapType < std::multimap<TKey, TValue> > (keytype.Get(), valuetype.Get());
		}
	};
	// Hash map
	template<typename TKey, typename TValue>
	struct DeduceType < UPS_HASH_MAP<TKey, TValue> > {
		IType* Get () {
			DeduceType<TValue> valuetype;
			DeduceType<TKey> keytype;
			return new MapType < UPS_HASH_MAP<TKey, TValue> > (keytype.Get(), valuetype.Get());
		}
	};
